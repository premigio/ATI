...
def akaze(image1: MyImage, image2: MyImage, octaves=None, threshold=500, show_detected_keypoints=True):
    akaze_cv2 = cv2.AKAZE_create(nOctaveLayers=octaves)
    return method(image1, image2, akaze_cv2, threshold, show_detected_keypoints)


def kaze(image1: MyImage, image2: MyImage, octaves=None, threshold=500, show_detected_keypoints=True):
...
def method(image1: MyImage, image2: MyImage, method, threshold, show_detected_keypoints):
    ...
    if show_detected_keypoints:
        ...
        plt.title("Detected points for: " + image1.file_name)
        plt.show()

        plt.imshow(img2_draw, cmap='gray')
        plt.title("Detected points for: " + image2.file_name)
        plt.show()
    ...
    final_img = cv2.drawMatches(img1, keypoints_1, img2, keypoints_2, matches, img2, flags=2)

    equal = len(matches) > threshold
